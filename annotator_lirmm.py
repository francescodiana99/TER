...
_API_KEY = "0d6cf805-5a44-4904-9c15-f8c3b971014f" # put your key here
...
def get_req_url(text, api_type='annotator', params=None, lang='fr'):
    ...
    if isinstance(params, dict) and params:
        # in caso params sia un dizionario corretto ed esista, vado ad aggiungere all'url i diversi parametri
...
def get_info_from_annot_by_term(annotations, filter='all'):
    ...
    # il CUI è il concept unique identifier, in pratica un codice usato per rappresentare unicamente un significato, che può magari essere espresso in modi diversi
...
if __name__ == "__main__":
    ...
    """ configurazione handler file di log, sono indicati rispettivamente:
    level: livello di eventi da loggare, in questo caso quelli informativi
    handler: in questo caso un handler che crea fino a 10 file da max 100000 byte e salva tutto nel file indicato
    format: formato con cui gestisce le stringhe
    daterm: formato per le date"""
    ...
    # Inputs
    SAVE_OUTPUT = True
    """path_txt contiene il path alla cartella che andrà a contenere l'output credo"""
    print(pathlib.Path().home().as_posix())
    path_txt = pathlib.Path().home().as_posix() + '/Desktop/uni/secondo anno/progetto/code/data/text'
    #path_txt = "/Desktop/uni/secondo anno/progetto/code/data/text"

    # Init
    files = [x for x in pathlib.Path(path_txt).iterdir() if x.suffix in ['.txt']]
    pathOut = pathlib.Path(path_txt)
    pathOut = pathOut.with_name(pathOut.stem+'_ann') # somewhere like path/to/path_txt_ann/
    ...
    for afile in files:
        ...
        if len(text_to_annotate)==0:
            logging.info(f'Empty file {afile.stem}, skipped')
...
# %%
prova = pd.read_csv('c:/Users/franc/Desktop/uni/secondo anno/progetto/code/data/text_ann/table_terms_onto.csv')
prova.style
