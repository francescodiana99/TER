...
def get_adr_and_sym(p):
    """
    This function aims to extract all the ADR and SYM from each paragraph, considering all as symptoms
    """
    sym = []
    for idx, r in enumerate(p.runs):
        # print(p.runs[0].text)
        ...
        if p.runs[0].text == r.text:
            prev = r
            if r.font.highlight_color == 16 or r.font.highlight_color == 4:
                sym.append(r.text)
        else:
            # here I check if the run is a part of a longer expression, checking if the color of the highlight is the same of the previous run
            if ((r.font.highlight_color == 16 and prev.font.highlight_color == 16) or (r.font.highlight_color == 4 and prev.font.highlight_color == 4)):
                # if the condition is true, I append the text because it is part of the same expression
                sym[-1] = sym[-1] + r.text
            elif r.font.highlight_color == 16 or r.font.highlight_color == 4:
                sym.append(r.text)
                # check if the element is the last one
            elif r.text == " " and p.runs[idx] != p.runs[-1]:
                # in case it is a space between two highlighted elements we added it to the list
                if (p.runs[idx - 1].font.highlight_color == 16 and p.runs[idx + 1].font.highlight_color == 16) or (p.runs[idx - 1].font.highlight_color == 4 and p.runs[idx + 1].font.highlight_color == 4):
                    sym.append(r.text)
            prev = r
    # now if there is a space in the list, we merge the text
    for idx, e in enumerate(sym):
    ...
    for e in sym:
        ...
        annotations.append((start + index, end + index, "SYM"))
        # save the index of the substring in p
        index = index + end
        # cut the part of p before the entity found
        temp = p.text[index:]
...
def preprocessing_alt(doc):
    """
    Alternative version of the preoprocessing function. This one extracts and labels only DRUGS and SYMPTOMS
    """
    training_data = []
    for p in doc.paragraphs:
        drugs = get_drugs(p=p)
        sym = get_adr_and_sym(p=p)
        ann = drugs + sym
        training_data.append((p.text, ann))
...
if __name__ == "__main__":
    ...
    training_data = preprocessing_alt(document)
    nlp = spacy.blank('fr')
    print(training_data)
